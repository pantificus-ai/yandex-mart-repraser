...
- Выгрузка локального списка товаров и цен в Excel (`GET /products/export`).
- Выгрузка каталога товаров и цен напрямую из API Яндекс Маркета (`GET /products/export/yandex-api`).
- Веб-интерфейс (`GET /`) с:
  - таблицей товаров,
  ...
  - отдельным блоком «Экспорт данных» с явной ссылкой-кнопкой выгрузки товаров и цен по API.
...
5. **API-выгрузка каталога** — прямой запрос в Partner API с пагинацией по товарам и ценам, затем объединение данных в Excel.
6. **Готовность к росту** — можно заменить SQLite на PostgreSQL через `DATABASE_URL`.
...
YANDEX_OFFER_INFO_PATH=/v2/businesses/{business_id}/offer-mappings
YANDEX_OFFER_PRICES_PATH=/v2/businesses/{business_id}/offer-prices
...
> Для API-выгрузки и синхронизации обязательно задайте `YANDEX_API_TOKEN` и `YANDEX_BUSINESS_ID`. Если они не заполнены, сервис вернет понятную ошибку конфигурации вместо падения `httpx` с `Bearer `.

## Что содержит API-выгрузка

Файл `yandex_market_api_products.xlsx` содержит:
- `vendor_code`,
- `name`,
- `vendor`,
- `category`,
...
- локальные `rrp`, `min_price` и `local_updated_at`, если артикул есть в базе репрайсера.
