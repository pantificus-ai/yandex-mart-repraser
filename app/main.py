...
input,button{padding:8px;border:1px solid #bbb;border-radius:6px}button{cursor:pointer;background:#111;color:#fff}
...
<button onclick='downloadApiExcel()'>Выгрузить товары и цены по API</button>
<input type='file' id='excelFile' accept='.xlsx,.xls' />
<button onclick='uploadExcel()'>Загрузить Excel</button>
</div>
<p class='small' id='status'>Готово</p>
<p class='small hint'>Кнопка «Выгрузить товары и цены по API» запрашивает актуальный каталог и цены напрямую из Yandex Market Partner API.</p>
</div>
<div class='card'>
<div class='filters'>
...
function downloadApiExcel(){window.location='/products/export/yandex-api'}
