...
class ProductStorage:
    ...
    async def update_market_prices(self, prices_by_vendor_code: dict[str, Decimal | dict | None]) -> None:
        now = datetime.now(timezone.utc)
        async with self._session_factory() as session:
            for vendor_code, market_price in prices_by_vendor_code.items():
                ...
                if row is None:
                    continue
                price_value = market_price.get('market_price') if isinstance(market_price, dict) else market_price
                row.market_price = Decimal(price_value) if price_value is not None else None
                row.updated_at = now
            await session.commit()
