...
class YandexMarketConfigurationError(ValueError):
    pass
...
class YandexMarketClient:
    ...
    def is_configured(self) -> bool:
        return bool(self._token and self._business_id > 0)

    async def close(self) -> None:
        await self._client.aclose()

    async def update_prices(self, items: list[dict]) -> None:
        self._ensure_configured(operation='обновления цен')
    ...
    async def get_market_prices(self, vendor_codes: list[str]) -> dict[str, dict[str, Decimal | str | None]]:
        ...
        self._ensure_configured(operation='получения цен')
    ...
    async def export_catalog_with_prices(self) -> list[dict[str, Any]]:
        self._ensure_configured(operation='выгрузки товаров и цен по API')
    ...
    def _ensure_configured(self, operation: str) -> None:
        if not self._token:
            raise YandexMarketConfigurationError(
                f'Для {operation} заполните YANDEX_API_TOKEN в файле .env или переменных окружения.'
            )
        if self._business_id <= 0:
            raise YandexMarketConfigurationError(
                f'Для {operation} заполните корректный YANDEX_BUSINESS_ID в файле .env или переменных окружения.'
            )
